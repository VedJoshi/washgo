    ...
    <Card className="border-ink/8 p-5 sm:p-6">
      <div className="flex flex-col gap-5">
        <div className="space-y-3">
          <Badge tone={tone}>
            {recommendation.urgency} {uiLanguage === 'vi' ? 'uu tien' : 'urgency'}
          </Badge>
          <div>
          ...
          <Link to="/booking" onClick={() => setSelectedRecommendationId(recommendation.id)} className="inline-flex">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="rounded-[22px] bg-sand/70 px-4 py-4">
            <p className="text-[11px] uppercase tracking-[0.18em] text-ink/45">{uiLanguage === 'vi' ? 'Van de hien tai' : 'What is wrong'}</p>
            <p className="mt-2 text-sm leading-6 text-ink/80">{recommendation.issue}</p>
          </div>
          <div className="rounded-[22px] bg-sky/45 px-4 py-4">
            <p className="text-[11px] uppercase tracking-[0.18em] text-ink/45">{uiLanguage === 'vi' ? 'Vi sao quan trong' : 'Why it matters'}</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-[22px] border border-ink/8 bg-white px-4 py-3">
            <p className="text-[11px] uppercase tracking-[0.18em] text-ink/45">{uiLanguage === 'vi' ? 'Can thuc hien' : 'Action to take'}</p>
            <p className="mt-2 text-base font-semibold text-ink">{recommendation.actionLabel}</p>
          </div>
          <div className="rounded-[22px] border border-ink/8 bg-white px-4 py-3">
            <p className="text-[11px] uppercase tracking-[0.18em] text-ink/45">{uiLanguage === 'vi' ? 'Thoi han' : 'By when'}</p>
            <p className="mt-2 text-base font-semibold text-ink">
              {uiLanguage === 'vi' ? 'Trong' : 'Within'} {recommendation.recommendedWithinDays} {uiLanguage === 'vi' ? 'ngay' : 'days'} - {recommendation.estimatedPriceRange}
            </p>
          </div>
        </div>
      </div>
